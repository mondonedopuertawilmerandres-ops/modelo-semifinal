...
El modelo se entrena **una sola vez** sobre el historial 1872-2026. Octavos
y Cuartos de Final ya se jugaron por completo (resultado real, incorporado a
`data/results.csv` y `data/matches.csv` a partir de fuentes públicas tras
jugarse cada partido), así que el modelo no los predice: se leen tal cual.
La única ronda que la cascada efectivamente predice es **Semifinales**
(`src/core/bracket.py` arma ese cruce a partir de los 4 clasificados reales
de Cuartos). Todos esos resultados reales -- los 8 de Octavos y los 4 de
Cuartos -- también se suman como **partidos de entrenamiento reales**, así
el modelo tiene más filas de historial genuino para calcular forma reciente,
H2H y validación temporal.
...
3. **Una vez medida la calidad, se reentrena con el 100% de los datos** (los
   7.623 partidos completos, ya con los 8 resultados reales de Octavos y los
   4 de Cuartos incluidos) para el modelo que efectivamente se usa a la hora
   de predecir los 2 cruces de Semifinales. La métrica de validación
   (1.0056) es la mejor estimación honesta de qué tan bien generalizará ese
   modelo final.
...
| Partidos de Cuartos de Final ya jugados (resultado real, no se predicen) | 4 (los 4) |
| Cruces de Semifinales armados a partir de esos 4 clasificados (única ronda que predice el modelo) | 2 |
...
## 4. De Cuartos a Semifinales: la única ronda que el modelo predice

Octavos y Cuartos de Final ya se jugaron por completo y sus 12 resultados
están cargados en `data/results.csv` / `data/matches.csv` /
`data/shootouts.csv`, así que el modelo no los predice: Francia, España,
Inglaterra y Argentina son los 4 clasificados reales de Cuartos.

`src/core/bracket.py` lee esos 4 ganadores directamente del dataset
(`ganador_final`, sin pasar por la cascada) y arma las 2 llaves de
Semifinales siguiendo el bracket oficial (Francia vs España el 14/7 en
Arlington, Argentina vs Inglaterra el 15/7 en Atlanta). Esas 2 filas nuevas
son las únicas que pasan por la predicción completa (espejo + Monte Carlo)
-- el modelo no distingue si el partido ya se jugó o es futuro, solo ve las
mismas 19 variables de siempre, pero en este proyecto solo se le pide que
las calcule para Semifinales.
